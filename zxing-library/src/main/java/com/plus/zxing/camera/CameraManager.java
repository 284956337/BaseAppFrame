...
public final class CameraManager {
	 ...
	 */
	public void openDriver(SurfaceHolder holder, int CAMERA_WHAT)
			throws IOException {
		if (camera == null) {
			if (CAMERA_WHAT == 0) {// 后置
				camera = Camera.open();
			} else if (CAMERA_WHAT == 1) {// 前置
		...
		try {
			if (framingRect == null) {
				...
				if (height < MIN_FRAME_HEIGHT) {
					...
					// height = MAX_FRAME_HEIGHT;//disable by xuqingfeng
					height = height / 2;// add by xuqingfeng
				}
